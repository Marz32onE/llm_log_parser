...
5. **Timestamps dominate.** A full ISO timestamp is ~16 tokens per line, the
   short clock ~8 — factoring the shared date into the pod header cut 25%
   alone, losslessly.
6. **Reference indirection hurts LLM comprehension.** logzip's two-level
...
7. **Aggregation beats per-line encoding for comprehension.** An LLM asked
