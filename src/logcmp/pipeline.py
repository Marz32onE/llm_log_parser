...
from logcmp.compressors import Compressor, Drain3Compressor, LogzipCompressor
...
LogRows = Sequence[PodLogRecord | Mapping[str, object]] | str
...
def compress_logs(
    ...
    **kwargs: object,
    ...
    Args:
        rows: Pod log rows with schema ``time``, ``pod_name``, ``message``.
            Accepts:
            - ``list[PodLogRecord]`` or ``list[dict]`` from a ClickHouse query
    ...
    return compress_text(records_to_text(records), len(records), algorithm, **kwargs)
