...
# compare + JSON 報告 + artifacts(報告含 LLM token 指標)
...
以下數據皆來自一份 629 行、3 個 pod 的真實事故樣本(穩定流量 → DB 變慢
→ connection pool 耗盡 → 504 → worker OOM → restart → 恢復),
以 tiktoken `o200k_base` 計量:
...
src/llmlogs/
  ...
  tokens.py                # 可選的 tiktoken token 計量
...
- 指標同時回報 UTF-8 bytes **與** LLM tokens(安裝 `tiktoken` 時 —
  `pip install llmlogs[tokens]`)。請以 token 數選擇格式,不要看 bytes —
  見[實測發現](#實測發現--以-llm-token-為準不是-bytes)。
