...
TSV/CSV 引號規則。最常見的參數化模板會在 `[body default=N]` header
宣告一次,其資料行直接省略 id(以 tab 開頭)— 在重複性高的 log 上,
...
預設(`with_preamble=False`)會直接從 `drain3-llmlogs-v3` 標記行開始:
...
2. **JSON 外殼是 token 毒藥。** 舊版 drain3 JSON v1 payload 逐行的
   ...
   現行的 TSV wire format(v2、現為 v3 — 見上方 `drain3` 範例)已經
   拿掉這層重複外殼,但這個數字是 TSV 上線前量測的 — 請用你自己的
   tokenizer 對現行輸出重新量測,別直接沿用這個數字。
3. **分隔符幾乎不影響 token,重複結構才影響。** 以 `o200k_base` 對
   600 行重複樣本量測,body 換成 comma/semicolon/pipe/space 都落在
   tab 的 ±0.1% 內 — 但省略最常見模板的 id(v3 的 `[body default=N]`
