...
**`drain3`** — `compress_logs(pods, "drain3")`:挖掘出的模板放進 JSON
legend;每行變成模板 id + 參數列表(可重建):
...
拆出多付的 token 花在哪。渲染文字 25,954 tokens;drain3 payload 28,960
(+11.6%)— 同一份 payload 以 chars 計反而*小* 3.6%:
