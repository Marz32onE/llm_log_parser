...
含 `time, pod_name, message` 的扁平 ClickHouse rows(或 JSON/NDJSON 字串)先用 `parse_pod_logs` 轉換一次;之後所有壓縮/digest 入口一律只收 `list[PodLogs]`。
...
**`drain3`** — `compress_logs(pods, "drain3")`:挖掘出的模板放進以逗號
分隔的 legend;每行變成模板 id + 參數列表(可重建),欄位採標準
CSV 引號規則。最常見的參數化模板會在 `[body default=N]` header
宣告一次,其資料行直接省略 id(以逗號開頭)— 在重複性高的 log 上,
比每行重複 id 約省 3–5% tokens。以下範例開啟了選用的說明前導區塊
(`Drain3Compressor(with_preamble=True)` 或
`compress_logs(pods, "drain3", with_preamble=True)`),用來就地解釋格式;
預設(`with_preamble=False`)會直接從 `drain3-llmlogs-v4` 標記行開始:
...
2. **JSON 外殼是 token 毒藥。** 舊版 drain3 JSON v1 payload 逐行的
   ...
   現行的表格 wire format(TSV v2/v3、現為 CSV v4 — 見上方 `drain3`
   範例)已經拿掉這層重複外殼,但這個數字是表格格式上線前量測的 —
   請用你自己的 tokenizer 對現行輸出重新量測,別直接沿用這個數字。
3. **分隔符幾乎不影響 token,重複結構才影響。** 以 `o200k_base` 對
   600 行重複樣本量測,body 換成 comma/semicolon/pipe/space 都落在
   tab 的 ±0.1% 內 — 但省略最常見模板的 id(`[body default=N]`
   規則)省了 3–5%。而且 log 內文常有逗號,comma 反而更常觸發 CSV
   引號包裹;v4 仍採 comma,方便試算表開啟。
4. **空白切詞的模板挖掘在 `key=value` log 上省不到什麼。** drain3 把
   `duration_ms=45` 當成單一詞,抽出的參數仍帶著 key,模板幾乎沒有
   factored out 任何東西。
5. **時間戳是大宗。** 完整 ISO 時間戳每行約 16 個 token,短時鐘約 8 個 —
   單是把共享日期抽進 pod header 就無損省下 25%。
6. **參照間接性傷害 LLM 理解。** logzip 的兩層 legend(`#10# = #0# #2#`)
   與 token 拼接(`#7#9` 代表 `duration_ms=3` + `9` → `duration_ms=39`)
   迫使 LLM 每行做多跳解碼。它的優點:罕見的關鍵行(OOM、restart)因為
   模板只捕捉高頻模式而保持原文。
7. **聚合勝過逐行編碼。** 被問「這個 pod 發生什麼事?」的 LLM 需要的是
...
verbatim「罕見」行。`DigestOptions(sim_th=0.25)` 把它們收成單一
